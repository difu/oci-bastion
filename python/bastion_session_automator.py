...
CLI = argparse.ArgumentParser(
    description="Usage: python3 bastion_session_automator.py -b BASTION_HOST_OCID -l OCI_PRIVATE_IP_1 DESTINATION_PORT_1 -l OCI_PRIVATE_IP_2 DESTINATION PORT_2 -r")
CLI.add_argument("--bastion_ocid", "-b", type=str, help="add your Bastion OCID")
CLI.add_argument("--run_forever", "-r", action='store_true',
                 help="Set this flag to create bastion sessions indefinitely")
CLI.add_argument("--local_connections", "-l", type=str, action="append", nargs="+",
                 help="USAGE: -l OCI_PRIVATE_IP OCI_PORT")
args = CLI.parse_args()
...
try:
    ...
    print(
        "ERROR -- The 'user_bastion_host_ocid' variable needs to be set, or you need to specify -b BASTION_HOST_OCID from the CLI.")
    raise SystemExit
try:
...
try:
    ...
    print(
        "If you are using Windows, make sure you are using 64 Bit Python and that OpenSSH is installed on your machine")
...
def commands(type, ip_addr, remote_port, privatekey):
    ...
    if type == "SOCKS5":
        ...
        cmd = "ssh", "-i", privatekey, "-o", "identitiesOnly=yes", "-o", "HostKeyAlgorithms=ssh-rsa", "-o", "PubkeyAcceptedKeyTypes=ssh-rsa", "-o", "serveraliveinterval=60", "-o", "StrictHostKeyChecking=no", "-N", "-D", "{}:{}".format(
            "127.0.0.1", local_port), "{}@{}".format(bastion_session_ocid, bastion_fqdn)
    if type == "LOCAL":
        print("{}:{} <--MAPPED TO--> localhost:{}".format(ip_addr, remote_port, local_port))
        cmd = ("ssh", "-i", privatekey, "-o", "identitiesOnly=yes", "-o", "HostKeyAlgorithms=ssh-rsa", "-o",
               "PubkeyAcceptedKeyTypes=ssh-rsa", "-o", "serveraliveinterval=60", "-o", "StrictHostKeyChecking=no", "-N",
               "-L", "{}:{}:{}".format(local_port, ip_addr, remote_port),
               "{}@{}".format(bastion_session_ocid, bastion_fqdn))
    return cmd
...
def exit_buddy():
    try:
        ...
        print("")
        print(
            "[Attention!] Uncomment the 'local_connections' variable or specify '-l' from the CLI to pass Non-SOCKS5 traffic.")
    cleanup()
...
async def subprocess(cmds, bastionocid):
    ...
    if subprocess_errors == 15:
        print(
            "ERROR! -- There is a configuration issue with SSH. I recommend resolving the SSH issue outside of the script before continue to use it. Here is the error.")
...
class ssh_keypair:
    ...
    def _generate_keypair(self):
        if self.user_public_key_path is None:
            ...
            self.printer = (
                "SSH KEY -- Using the SSH keypair that was specified in the script. Encrypted keys are not currently supported in this script.")
        if self.user_public_key_path and self.user_private_key_path:
            try:
                ...
                print(
                    "Unset the public and private keypaths to have the script automatically create an SSH RSA keypair.")
                print("")
                print(e)
                raise SystemExit
        else:
...
try:
    ...
    if region_set is None:
        print(
            "ERROR -- Unable to set the region for this Bastion Host. Make sure the Bastion OCID was copied properly.")
        raise SystemExit
    oci.config.validate_config(oci_config)
...
for ip_block in allowed_cidrs:
    if ipaddress.ip_address(client_public_ip) in ipaddress.ip_network(ip_block):
        if ipaddress.ip_network(ip_block) == ipaddress.ip_network("0.0.0.0/0"):
            print("Bastion Host -- CIDR ALLOW Rule '{}' will allow connectivity from PUB IP {}".format(ip_block,
                                                                                                       client_public_ip))
...
if allow_all_cidrs is None:
    ...
    if foundcidr is None:
        ...
        print(
            "Bastion Host -- This script will update the CIDR with your Public IP address and assume you have the permissions to update the Bastion Host.")
        print(
            "Bastion Host -- If you continue to see this message after rerunning the script, you most likely don't have permissions to add your CIDR to the allow list.")
        allowed_cidrs.append(str(host_pub_ip))
    ...
    print("Bastion Host -- The Bastion Host ALLOW LIST has been configured to accept connections from {}.".format(
        client_public_ip))
...
if len(active_bastion_sessions) >= int(bastion_host.data.max_sessions_allowed * .8):
    print("Bastion Host -- Session Count is Nearing the Limit of the Bastino Host -- {}/{}".format(
        len(active_bastion_sessions), bastion_host.data.max_sessions_allowed))
...
if len(active_bastion_sessions) == bastion_host.data.max_sessions_allowed:
    print(
        "Bastion Host -- Bastion Host is at capacity - ACTIVE{}/TOTAL{} SESSIONS. A session needs to be freed up before you can connect to this Bastion.".format(
            len(active_bastion_sessions), bastion_host.data.max_sessions_allowed))
    raise SystemExit
print("Bastion Host -- {} Total Active Sessions = {}".format(bastion_host.data.name.upper(),
                                                             len(active_bastion_sessions)))
...
try:
    ...
    if user_session_ttl > bastion_host.data.max_session_ttl_in_seconds:
        ...
        print("Bastion Session -- Setting Session Timeout to Max Bastion Timeout, {}".format(
            bastion_host.data.max_session_ttl_in_seconds))
            ...
            try:
                ...
                if session_state.data.lifecycle_state != "ACTIVE":
                    ...
                    if access_still_allowed is None:
                        print(
                            "ERROR -- Ending this session since the Bastion Host's CIDR allowlist does not allow your public IP.")
